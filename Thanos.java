...
public class Thanos {
    ...
    public static void main(String[] args) {
        ...
        fighter.add (new Hero ("Black Widow", 34));
        fighter.add (captainAmerica);
        fighter.add (vision);
        fighter.add (ironMan);
        fighter.add (scarletWitch);
        fighter.add (new Hero ("Thor", 1500));
        fighter.add (hulk);
        fighter.add (doctorStrange);


        // TODO 3 : It's Thor birthday, now he's 1501
        fighter.get(5).setAge (1501);
        ...
        for (int k = 0; k <= fighter.size()/2+1; k++){
          //System.out.println("-" + fighter.get(k).getName());

          fighter.remove(k);
        }*/
        /*List<Hero>sublist = fighter.subList(0, fighter.size()/2);
        sublist.clear();*/
        fighter.subList(0, fighter.size()/2).clear();
